...
|-------|-------------|----------|---------|
| **Main** | Set the context-management limit for goose | LiteLLM proxies and deployments whose runtime metadata is unavailable | `GOOSE_CONTEXT_LIMIT` |
| **Ollama runtime** | Set the context window allocated by Ollama (`num_ctx`) | Local Ollama models | `GOOSE_INPUT_LIMIT` |

:::info
`GOOSE_CONTEXT_LIMIT` controls goose context management, including compaction and displayed usage. It does not change provider runtime allocation. For Ollama, use `GOOSE_INPUT_LIMIT` to configure `num_ctx`.
:::
...
2. Ollama with a larger runtime context window

```bash
export GOOSE_INPUT_LIMIT=500000
```
